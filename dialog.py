...
class SpeechCloudWS(websocket.WebSocketHandler, EventEmitter):
    ...
    @classmethod
    def run(cls, dialog_class, address, port, path="/ws", static_path=None, static_route="/static/(.*)"):
        app_config = [
            (path, cls, {"dialog_class": dialog_class}),
            ]

        if static_path is not None:
            app_config.append( (static_route, web.StaticFileHandler, {"path": static_path}) )
...
class Dialog(object):
    ...
    async def synthesize(self, text, **kwargs):
        if not isinstance(text, list):
            text = [text]
        for i in text:
            await self.sc.tts_synthesize(text=i, **kwargs)
        
    async def synthesize_and_wait(self, text, **kwargs):
        if not isinstance(text, list):
            text = [text]
        for i in text:
            # call tts_synthesize multiple-times to buffer the TTS result
            await self.sc.tts_synthesize(text=i, **kwargs)
        for _ in text:
            # await the corresponding number of tts_done messages
    ...
    async def display(self, text):
        self.logger.debug(f"DISPLAY: {text}")
        return await self.sc.dm_display(text=text)
